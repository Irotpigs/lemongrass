...
Version: 1.0
Project Boon
This Parser belongs to a third party package. (not lemongrass)
*******************************/
...
public class INIParser
{
  #region "Declarations"
    ...
    public static string applicationConfigPath { get; private set; } = Application.dataPath + "/Config/";
    ...
    // *** Local cache ***
    private IniDictionary m_Sections = new IniDictionary();
    private IniDictionary m_Modified = new IniDictionary();
    ...
    // Returns the recently parsed section information.
    // This information can't be guaranteed to be up to date. This is only what was parsed by the ini at the time of opening
    public IniDictionary GetCachedSections()
    {
        return new IniDictionary(m_Sections);
    }
    ...
    private string ReadValue_Internal(string SectionName, string Key, string DefaultValue)
    {
        lock (m_Lock)
        {
            // *** Check if the section exists ***
            Dictionary<string, string> Section;
            if (!m_Sections.TryGetValue(SectionName, out Section)) return null;
    ...
    {
        string StringValue = ReadValue_Internal(SectionName, Key, DefaultValue);
        if (!string.IsNullOrEmpty(StringValue)) return StringValue;
        WriteValue(SectionName, Key, DefaultValue);
        return DefaultValue;
    }

    public bool ReadValue(string SectionName, string Key, bool DefaultValue)
    {
        string StringValue = ReadValue_Internal(SectionName, Key, DefaultValue.ToString(System.Globalization.CultureInfo.InvariantCulture));
        if (string.Compare(StringValue, "true", true) == 0) return true;
        if (string.Compare(StringValue, "false", true) == 0) return false;
        WriteValue(SectionName, Key, DefaultValue);
        return DefaultValue;
    }
    ...
    {
        string StringValue = ReadValue_Internal(SectionName, Key, DefaultValue.ToString(CultureInfo.InvariantCulture));
        int Value;
    ...
    {
        string StringValue = ReadValue_Internal(SectionName, Key, EncodeByteArray(DefaultValue));
        try
        {
